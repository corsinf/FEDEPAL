...
// IDs de las carpetas raíz con sus nombres (ahora como arrays para permitir múltiples IDs)
$rootFolders = [
	"MARZO" => ["1ZH-p4H-UI2K1wdsODLRcSVlMSlSK3wRv"],
	"ABRIL" => ["1lBiRjcHYBkLS-L_dyRJxCPtRFwtLj2RQ"],
	"MAYO" => ["178XpyFebDMEugvflq3m9BTZrNVyQfjg0"],
	"JUNIO" => ["1KBqpKVf3kxTUH_4F6EHLn0OtRHdQTrLn", "1ZSSD2n8Yoap1P-ox-VnARvzfB_Kt8vCo"],
	"AGOSTO" => ["1LBAa9M63ZNKKuJD-fAW5Jp0Ntqu_fph0"],
	"OCTUBRE" => ["1XSCPGkBknujAMB-z7hh81yvFiOmQnq7A"]
];

// Función para asignar el icono de video si es un archivo de video
function seleccionarIcono($mimeType)
{
	// Lista de tipos MIME para videos
	$videoMimeTypes = [
		"video/mp4",
		"video/x-matroska",
		"video/quicktime",
	...
	if (in_array($mimeType, $videoMimeTypes)) {
		return "🎥"; // Icono de video
	}

	return "📄"; // Icono predeterminado para otros archivos
}

// Función recursiva para listar carpetas y archivos en Google Drive
function listFilesAndFolders($service, $folderId)
{
	try {
		...
		// Luego imprimir archivos
		foreach ($files->getFiles() as $file) {
			$mimeType = $file->getMimeType();
			$fileUrl = "https://drive.google.com/file/d/{$file->getId()}/preview";

			// Obtener el icono correcto según la extensión
			$icon = seleccionarIcono($mimeType);

			// Mostrar el archivo con su icono
			$output .= "<li>$icon <a href='#' onclick='visualizarArchivo(\"$fileUrl\")'>{$file->getName()}</a></li>";
		}
...
$fileLists = [];
foreach ($rootFolders as $folderName => $folderIds) {
	$fileLists[$folderName] = "";
...
<head>
	<title>Capacitaciones - Fedepal</title>
	<meta charset="utf-8" />
	...
	<style>
		/* Contenedor principal que alinea sidebar y visor */
		#imContent {
			display: flex;
			align-items: stretch;
		...
		/* Asegurar que el header esté en una sola línea */
#imHeaderObjects {
    display: flex;
    justify-content: space-between;
    align-items: center;
...
/* Asegurar que el menú ocupe espacio sin desajustes */
#imHeader_imMenuObject_02 {
    flex-grow: 1;
    text-align: center;
}

/* Mantener los elementos del menú en una sola línea */
#imHeader_imMenuObject_02 ul {
    display: flex;
...
#imHeader_imMenuObject_02 ul li {
    display: inline-block;
}

/* Asegurar que el botón de contactos esté alineado */
#imHeader_imMenuObject_02 ul li:last-child {
    margin-right: 20px;
}

/* Asegurar que el botón de Contactos no se desplace */
#imHeader_pluginAppObj_04 {
    white-space: nowrap;
    margin-left: auto;
}

/* Estilo del botón de Contactos */
#imHeader_imMenuObject_02 ul li a.contact-button {
...
/* Hover del botón de Contactos */
#imHeader_imMenuObject_02 ul li a.contact-button:hover {
    background-color: #e68d24;
}


#imHeader_pluginAppObj_04 a:hover {
    background-color: #e68d24;
}
		...
		.viewer {
			width: 50vw;
			/* Ocupa la otra mitad de la pantalla */
			min-width: 50%;
			max-width: 50%;
			display: flex;
			justify-content: center;
		...
		/* Asegurar que el iframe ocupa todo el espacio del visor */
		iframe#viewer-frame {
			width: 100%;
			height: 100%;
			border: none;
		...
		/* Eliminar márgenes extra del body */
		html,
		body {
			margin: 0;
			padding: 0;
...
<body> <!-- Google Tag Manager (noscript) -->
	...
	<div id="imPageExtContainer">
		<div id="imPageIntContainer">
			<div id="imPage">
				<header id="imHeader">
					<h1 class="imHidden">Capacitacion - Fedepal</h1>
				...
				<div id="imStickyBarContainer">
					...
					<div id="imStickyBar">
						<div id="imStickyBarObjects">
							<div id="imStickyBar_imObjectImage_01_wrapper" class="template-object-wrapper">
								<div id="imStickyBar_imObjectImage_01">
									<div id="imStickyBar_imObjectImage_01_container"><a href="home.html" onclick="return x5engine.utils.location('home.html', null, false)"><img src="images/cropped-logosmall.svg" title="" alt="" width="198" height="92" />
										</a></div>
								</div>
							</div>
							<div id="imStickyBar_imMenuObject_02_wrapper" class="template-object-wrapper"><!-- UNSEARCHABLE -->
								...
								<script>
									...
									x5engine.boot.push(function() {
										x5engine.initMenu(imStickyBar_imMenuObject_02_settings)
									});
									$(function() {
										$('#imStickyBar_imMenuObject_02_container ul li').not('.imMnMnSeparator').each(function() {
											var $this = $(this),
												timeout = 0;
											$this.on('mouseenter', function() {
												if ($(this).parents('#imStickyBar_imMenuObject_02_container-menu-opened').length > 0) return;
												clearTimeout(timeout);
												setTimeout(function() {
													$this.children('ul, .multiple-column').stop(false, false).fadeIn();
												}, 250);
				...
				<div id="imSideBar">
					<div id="imSideBarObjects">
						<div id="imSideBar_imObjectImage_01_wrapper" class="template-object-wrapper">
							<div id="imSideBar_imObjectImage_01">
								<div id="imSideBar_imObjectImage_01_container"><img src="images/empty-GT_imagea-1-.webp" title="" alt="" width="140" height="140" />
								</div>
							</div>
						</div>
					</div>
				...
				<main id="imContent">
					<!-- Sidebar con la lista de archivos y carpetas -->
					<div class="sidebar">
    <h1 style="font-size: 24px; font-weight: bold;">CAPACITACIONES</h1>
    <?php foreach ($fileLists as $folderName => $fileList): ?>
        <h2 style="font-size: 20px; font-weight: bold; color: #e68d24;">📂 <?php echo $folderName; ?></h2>
